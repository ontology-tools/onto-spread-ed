...
import { ProgressIndicator, type Release } from "@ontospreaded/js-core";
...
<template>
  ...
  <template v-if="release.state === 'waiting-for-user' && data?.errors?.length > 0">
    <div class="alert alert-danger">
      <h4>Errors occurred while publishing to AddictOVocab</h4>
  ...
  <ProgressIndicator v-else :details="data" :release="release" stepName="ADDICTO_VOCAB">
    <p>
  ...
  <template v-if="release.state === 'completed'">
    <p>
      The ontologies were published to AddictOVocab.
    </p>
  </template>

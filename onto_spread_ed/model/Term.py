...
class Term:
    ...
    equivalent_to: list[TermIdentifier]
...
class UnresolvedTerm:
    ...
    equivalent_to: list[TermIdentifier] = field(default_factory=list)
    ...
    def is_unresolved(self) -> bool:
        return any(v is None for v in [self.id, self.label, self.origin]) or \
            len(self.disjoint_with) + len(self.sub_class_of) + len(self.equivalent_to) > 0 and \
            any(t.is_unresolved() for t in [*self.sub_class_of, *self.equivalent_to, *self.disjoint_with])

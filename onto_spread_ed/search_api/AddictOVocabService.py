...
class AddictOVocabService(APIService):
    _api_client: AddictOVocabClient
    ...
    def __init__(self, config: dict, session: aiohttp.ClientSession):
        ...
        authtoken = config.get(PROP_ADDICTO_VOCAB_API_AUTH_TOKEN,
                               os.environ.get(PROP_ADDICTO_VOCAB_API_AUTH_TOKEN, None))

        self._config = config

        self._api_client = AddictOVocabClient(path, session, authtoken, config.get("DEBUG", False))
